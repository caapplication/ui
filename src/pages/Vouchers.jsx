...
const Vouchers = ({ selectedOrganisation, selectedEntity, isDataLoading, onRefresh, isActive = true, entities: entitiesFromProps }) => {
  ...
  */
  ...
  const handleViewVoucher = (voucher, hasFilters, sortedFilteredVouchers) => {
    // Preserve the exact list order from the table for the next/prev buttons
    navigate(`/vouchers/ca/${voucher.id}`, { state: { voucher, vouchers: sortedFilteredVouchers || enrichedVouchers, organisationId: selectedOrganisation } });
  };
