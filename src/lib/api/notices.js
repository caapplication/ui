...
export const getUnreadNoticeCount = async (token) => {
  try {
    ...
    // Calculate total including chats and closure requests
    const total = (data.unread_comments || 0) + (data.closure_requests || 0);
    
    // Fallback to data.count if none of the above are present
...
export const getNoticeDashboardAnalytics = async (days, agencyId, token, entityId = null) => {
  let url = `${FINANCE_API_BASE_URL}/api/notices/dashboard-analytics?days=${days}`;
